...
int FileManager::WriteChunk(SharedPointer<HashTable<String, TermPostingList *>> termIndex,
                  ...
                  const char* docs_filename) 
   {
    ...
    int FileManager::ReadChunk(const char *path) {
        if(!path) {
            std::cerr << "Error openning file " << path << " with errno = " << strerror( errno ) << std::endl;
            return -1;
        }
        void * blob;
        
         int chunkFile = open( path, O_RDONLY, S_IRWXU | S_IRWXG | S_IRWXO );
        if (chunkFile == -1 ) {
            std::cerr << "Error openning file " << path << " with errno = " << strerror( errno ) << std::endl;
            return -1;
         }
...
// TODO: Make sure chunk is less than chunk num
// TODO: Pass chunk nums
// May return refernece
TermPostingListRaw FileManager::GetTermList(String &term, const char *chunk_path) {
    if(chunk_path) {
        ReadChunk(chunk_path);
    }
    if(!termIndexBlob) {
        throw "Error: No chunk has been read";
    }
    
    const SerialTuple * tuple = termIndexBlob->Find(term.cstr());
    if(!tuple) {
        return nullptr;
    }
    else {
...
EndDocPostingListRaw FileManager::GetEndDocList(const char *chunk_path) {
    if(chunk_path) {
        ReadChunk(chunk_path);
    }
    if(!endDocListBlob) {
