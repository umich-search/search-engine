...
int DiskQueue::openFileRD( int id )
    ...
    if ( fd == -1 )
        std::cerr << "Cannot open file " << filePath << " with errno = " << errno << std::endl;
    return fd;
    }
...
int DiskQueue::openFileWR( int id )
    ...
    String filePath = dirName;
    filePath += ltos( id );
    int fd = open( filePath.cstr( ), O_CREAT | O_WRONLY | O_APPEND );
    if ( fd == -1 )
        std::cerr << "Cannot open file " << filePath << " with errno = " << errno << std::endl;
    return fd;
    }
    ...
    if ( handle )
        ...
        while ( entry = readdir( handle ) )
            ...
            if ( stat( childPath.cstr( ), &statbuf ) )
                std::cerr << "stat of " << childPath << " failed, errno = " << errno << std::endl;
    ...
    if ( nread == -1 )  // EOF
        ...
        if ( readFileID + 1 == writeFileID )  // the next file is in writing mode
            ...
            if ( unlink( pathName.cstr( ) ) )  // delete the file
                std::cerr << "Error deleting the file " << pathName << " with the errno = " << errno << std::endl;
...
void DiskQueue::PushBack( String& item )
    ...
    if ( write( writeFd, item.cstr( ), item.size( ) ) == -1 )
        std::cerr << "Error writing to file " << item << " with errno = " << errno << std::endl;
