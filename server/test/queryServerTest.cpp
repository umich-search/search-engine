...
int main( )
    {
    std::string query1 = "Google | Facebook", query2 = "C++", query3 = "EECS";
    QueryServer qServer;
    try
        {
        ...
        printRanks( res1 );
        ::vector< url_score > res2 = qServer.CollectRanks( query2 );
        std::cout << "Results for query=" << query2 << std::endl;
        printRanks( res2 );
        ::vector< url_score > res3 = qServer.CollectRanks( query3 );
